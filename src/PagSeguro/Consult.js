...
class Consult {
	async getByReferenceCode(referenceCode) {
		...
		if (!referenceCode)
			return validations.mountReturn(
				validations.errorMsg(errorMsg(functionName, 'referenceCode'))
			);
	...
	async dateInterval(initialDate, finalDate, page = 1, maxPageResults = 50) {
		...
		if (!initialDate)
			return validations.mountReturn(
				validations.errorMsg(errorMsg(functionName, 'initialDate'))
			);
		if (!finalDate)
			return validations.mountReturn(
				validations.errorMsg(errorMsg(functionName, 'finalDate'))
			);
	...
	async getNotificationCode(notificationCode) {
		...
		if (!notificationCode)
			return validations.mountReturn(
				validations.errorMsg(errorMsg(functionName, 'notificationCode'))
			);

...
class Payment {
  ...
  async getPaymentMethods (amount, sessionId) {
    ...
    if (!amount) return validations.mountReturn(`amount not defined in ${functionName}`);
    if (!sessionId) return validations.mountReturn(`sessionId not defined in ${functionName}`);
  ...
  async getCardFlag (sessionId, bin) {
    ...
    if (!sessionId) return validations.mountReturn(`sessionId not defined in ${functionName}`);
    if (!bin) return validations.mountReturn(`bin not defined in ${functionName}`);
  ...
  async getCardToken (sessionId, amount, cardData) {
    ...
    if (!sessionId) return validations.mountReturn(`sessionId not defined in ${functionName}`);
    if (!amount) return validations.mountReturn(`amount not defined in ${functionName}`);
    if (!cardData) return validations.mountReturn(`cardData not defined in ${functionName}`);

    const cardDataValidated = validations.validateCardData(functionName, cardData);
    if (cardDataValidated.hasErrors) return validations.mountReturn(cardDataValidated.errors.join(', '));
  ...
  async getInstallments (sessionId, amount, creditCardBrand) {
    ...
    if (!sessionId) return validations.mountReturn(`sessionId not defined in ${functionName}`);
    if (!amount) return validations.mountReturn(`amount not defined in ${functionName}`);
    if (!creditCardBrand) return validations.mountReturn(`creditCardBrand not defined in ${functionName}`);
  ...
  async payment(payment, sender, items, receiver, shipping) {
    const shippingAddressRequired = shipping ? true : false;

    const functionName = 'payment';
    if (!EMAIL_PAGSEGURO || !TOKEN_PAGSEGURO) return {};
    if (!payment) return validations.mountReturn(`payment not defined in ${functionName}`);
    if (!sender) return validations.mountReturn(`sender not defined in ${functionName}`);
    if (!receiver) return validations.mountReturn(`receiver not defined in ${functionName}`);
    
    const myHeaders = new Headers();
    myHeaders.append('Content-Type', 'application/x-www-form-urlencoded');
    
    const paymentValidated = validations.validatePayment(functionName, payment);
    if (paymentValidated.hasErrors) return validations.mountReturn(paymentValidated.errors.join(', '));

    const senderValidated = validations.validateSender(functionName, sender);
    if (senderValidated.hasErrors) return validations.mountReturn(senderValidated.errors.join(', '));
    ...
    urlencoded.append('extraAmount', payment.extraAmount || 0.00);

    let i = 1;
    for (const item of items) {
      const itemValidated = validations.validateItem(functionName, item);
      if (itemValidated.hasErrors) return validations.mountReturn(itemValidated.errors.join(', '));
    ...
    if (shippingAddressRequired) {
      const shippingValidated = validations.validateShipping(functionName, shipping);
      if (shippingValidated.hasErrors) return validations.mountReturn(shippingValidated.errors.join(', '));
    ...
    if (payment.paymentMethod == 'creditCard') {
      ...
      if (cardValidated.hasErrors) return validations.mountReturn(cardValidated.errors.join(', '));
      ...
      urlencoded.append('noInterestInstallmentQuantity', process.env.NO_INTEREST_INSTALLMENT_QUANTITY || 2);
